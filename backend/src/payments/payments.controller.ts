...
export class PaymentsController {
  ...
  async initiatePayment(
    ...
    const result = await this.paymentsService.createTransaction(userId, planId, provider, phone);
    return {
  ...
  @Post('promo/validate')
  @HttpCode(HttpStatus.OK)
  async validatePromo(@Body('code') code: string, @Body('plan_id') planId: string) {
    return {
      valid: code.toUpperCase() === 'DJELIPROMO' || code.toUpperCase() === 'WELCOME',
    };
  }
