...
export class PaymentsService {
  ...
  async createTransaction(userId: string, planId: number, gateway: string, phoneMomo?: string) {
    const plan = await this.prisma.plan.findUnique({ where: { id: planId } });
    if (!plan || !plan.isActive) {
      throw new NotFoundException("La formule d'abonnement sélectionnée n'existe pas ou est inactive.");
    }
  ...
  async handleWebhook(gateway: string, body: any) {
    ...
    if (gatewayStatus === 'succeeded') {
      ...
      if (!payment.userId || !payment.planId) {
        throw new BadRequestException("Données de transaction incomplètes (userId ou planId manquant).");
      }
      ...
      await this.prisma.payment.update({
        ...
        data: {
          ...
          errorMessage: errorMessage || 'Paiement décliné par la banque ou solde mobile money insuffisant.',
        },
      });
