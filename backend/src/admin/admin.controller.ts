import { Controller, Get, Patch, Param, Delete, Post, Body, UseInterceptors, UseGuards, UploadedFiles, Inject, UnsupportedMediaTypeException } from '@nestjs/common';
...
const ALLOWED_MEDIA_MIME_TYPES = ['video/mp4', 'video/webm', 'video/quicktime', 'audio/mpeg', 'audio/mp4', 'audio/wav'];
...
export class AdminController {
  ...
  @UseInterceptors(FileFieldsInterceptor([
    ...
    fileFilter: (req, file, cb) => {
      const allowed = file.fieldname === 'cover' ? ALLOWED_COVER_MIME_TYPES : ALLOWED_MEDIA_MIME_TYPES;
      if (!allowed.includes(file.mimetype)) {
        return cb(new UnsupportedMediaTypeException(`Type de fichier non autorisé pour "${file.fieldname}": ${file.mimetype}`), false);
      }
      cb(null, true);
    },
    limits: {
